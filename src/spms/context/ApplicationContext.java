...
public class ApplicationContext {
    ...
    public void prepareObjectsByAnnotation (String basePackage) throws Exception { // basePackage 안에 있는 컴포넌트 어노테이션을 사용하는 객체의 정보를 받아오는 메서드
    ...
    public void prepareObjectsByProperties (String propertiesPath) throws Exception { // properties 파일에 있는 정보를 토대로 저장하는 메서드
        Properties props = new Properties();
        props.load(new FileReader(propertiesPath)); // propertiesPath 경로에 해당하는 파일을 읽어와서 "키 : 값" 으로 저장

        Context ctx = new InitialContext(); // context.xml의 정보를 가져올 Context 객체
        // key, value를 담을 객체를 준비
        String key = null;
        String value = null;
    ...
    public void injectDependency() throws Exception { // Hashtable 에 있는 객체 중 jndi 객체가 아닌 것들을 callSetter에 매개변수 값을 넣고 실행시킨다.
